...
{
    public partial class frm_SelectPlaces : Form
    {
        public frm_SelectPlaces()
        {
            try
            {
                InitializeComponent();
		...
		private void frm_SelectPlaces_Load(object sender, EventArgs e)
		{
			result = objManager.View_Places(null);
			if ( result.success )
			{
				...
				if (CurrentInfo != null)
					foreach ( int item in CurrentInfo )
					{
						...
						GridViewRowInfo FindRow =  radGridViewSearch.Rows.Single ( x => ( int ) x.Cells ["Place_Id"].Value == item );
		...
		private void rbtnSelectGate_Click( object sender , EventArgs e )
		{
			try
			{
				if ( CheckOneSelectedRow ( radGridViewSearch.SelectedRows.Count ) )
				{
					...
					{
						var value =int.Parse(radGridViewSearch.CurrentRow.Cells ["Place_Id"].Value.ToString());
						var query = radGridViewSelected.Rows.Where ( x => int.Parse ( x.Cells ["Place_Id"].Value.ToString () ) == value );
						if ( query.Count ()==0 )
						{
							radGridViewSelected.Rows.AddNew ();
		...
		internal void SetInfo( List<int> CurrentPlaces )
		{
			CurrentInfo = CurrentPlaces;
		}

		private List<int> CurrentInfo { get; set; }
