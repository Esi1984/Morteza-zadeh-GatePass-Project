...
{
    public partial class mortezazadehUcGate3 : UserControl
    {

        Manager GateManager = null;
        OutputDatas myCurrentState = null;

        decimal seePassingCode = -1;
      ...
      //  public string labelMyGate = "";
        ...
        public void GetNews()
        {
            ...
            bool power = false;
            var RNews = GateManager.EsiContCurrentState(mygate, LastRefreshTime, seePassingCode);
            if (RNews.success)
            {
                if (RNews.ResultTable != null)
                {
                    ...
                    power = (bool)myCurrentState.ResultTable.Rows[0]["EsiMiniGate_Power"];
                    if (power)
                    {
                    switch ((ItemsPublic.MiniGateMainEsiMode)(int)myCurrentState.ResultTable.Rows[0]["MiniGateMainEsiMode_Id"])
                    {
                        ...
                        case ItemsPublic.MiniGateMainEsiMode.Unkown:
                            this.lightsMainMode1.GateMainMode = ItemsPublic.MiniGateMainEsiMode.Unkown;
                             this.LightGoIn.DisableMode();
                            this.LightGoOut.DisableMode();
                            ...
                            DisablePassingView();
                            this.PicDisableGate.Visible = true;
                    ...
                    {
                        this.rtbGateState.Text =  "کنترولر خاموش می باشد";
                ...
                if (power && seePassingCode!= -1 && RNews.MiniTable1 != null)
                {
//                    ShowPassingView();

                    Byte[] dataPic = new Byte[0];//Person_Picture
                    if (!(RNews.MiniTable1.Rows[0]["Person_Picture"].Equals(System.DBNull.Value)))
                     {
                         dataPic = (Byte[])RNews.MiniTable1.Rows[0]["Person_Picture"];
                     }


                    bool ComeInBool = (int)RNews.MiniTable1.Rows[0]["PasserMortezazadeh_RealState"] == 5 ? true : false; ;

                    morMonitoring1.ShowPersonPass(RNews.MiniTable1.Rows[0]["Person_Name"].ToString().Trim()
                        , RNews.MiniTable1.Rows[0]["Person_Surname"].ToString().Trim()
                        ,ref dataPic
                        ,ComeInBool);

                    seePassingCode = (decimal)RNews.MiniTable1.Rows[0]["PasserMortezazadeh_ID"];
                }
        ...
        public bool TryToConnect() 
        {
            try
            {
                ...
                myCurrentState = GateManager.EsiContCurrentState(mygate, LastRefreshTime, seePassingCode);
                ...
                if (myCurrentState.success)
                {
                    ...
                    ItemsPublic.MiniGateMainEsiMode kkk =
                        (ItemsPublic.MiniGateMainEsiMode)
                        (int)myCurrentState.ResultTable.Rows[0]["MiniGateMainEsiMode_Id"];

                    if (kkk == ItemsPublic.MiniGateMainEsiMode.Unkown)
                    {
                        this.LightGoIn.DisableMode();
                        this.LightGoOut.DisableMode();
                    }

                    this.lightsMainMode1.GateMainMode = kkk;
        ...
        private void EnablePassingView()
        {

            seePassingCode = 0;
        //    PicDisableGate.Visible = false;

            PicEnableGate.Visible = false;
            rgbJustGate.Visible = false;

           //:=  PicPassingEsi.Visible = true;

            morMonitoring1.Visible = true;
        }

        private void ShowPassingView()
        {
            throw new NotImplementedException();
        }

        private void DisablePassingView()
        {
            seePassingCode = -1;
            PicEnableGate.Visible = false;
           // PicPassingfEsi.Visible = false;
        
            PicDisableGate.Visible = true;
            rgbJustGate.Visible = true;
            //:=     PicPassingEsi.Visible = false;
            //:=     PicPassingEsi.Image = global::aorc.gatepass.Properties.Resources.Male;
            morMonitoring1.Visible = false;


        }

        private void LightGoIn_Load(object sender, EventArgs e)
        {

        }

        private void morMonitoring1_EventEsiNoSeePass()
        {
            DisablePassingView();
            LastRefreshTime = null;
            GetNews();
