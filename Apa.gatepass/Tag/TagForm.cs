     ...
     //   UC_TagSearch ucSearchInOut;
        ...
        public TagForm()
        {
            ...
            InitializeComponent();
                ...
                clmReader = new MortezazadehReaders();
        ...
        private void cbbSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                CursorWait();
               if (ucSearchInOut.rtbGpId.Text.Trim().Count()<10)
	            {
                    CursorDefault();
		             return;
	            }
              ...
              //  uC_TreeReport1.Enabled = false;
                ...
                switch (CurrentState)
                {
                    ...
                    case _State._operator:
                        break;
                    case _State._Inout:
                        result = objManager.FindJustArchive( ucSearchInOut.rtbGpId.Text.Trim());
                        break;
                    default:
                        break;
                }
                if (result != null)
                    if (result.success)
                    {
                        ...
                        if (radGridViewReport.Rows.Count < 1)
                        {
                            ...
                            ucSearchInOut.rtbGpId.SelectAll();
                        }
                        else
                        {
                    ...
                    {
                        ItemsPublic.ShowMessage("انجام عملیات تخصیص تگ به دلیل زیر ناموفق بود" + "\r\n" + "\r\n" + result.Message);
                    }
                CursorDefault();
          ...
          //  toolWindowSecondProperties.Hide();
            ...
            switch (CurrentState)
            {
                ...
                case _State._Inout:
                  ...
                  //  aorc.gatepass.Complex_Reports.Reporter
                    ucPropertyInOut = new aorc.gatepass.Complex_Reports.ReportInOut.UC_PageGp();
                    // ucPropertyPerson = new UC_ViewPersonDetails();
                    // ucPropertyVeh = new ui.vehicle.UC_vehicleDetails3();

                 //   ucSearchInOut = new UC_TagSearch();
...
gridViewDecimalColumnVeh1
        ...
        public void textha (){
            ...
            int kk = 0;
            string kkkk;
    while (kk< ucSearchInOut.rtbGpId.Text.Count())
	{
        kk++;
	        kkkk =   ucSearchInOut.rtbGpId.Text.Substring(kk);
	}
        ...
        private void TagForm_Shown(object sender, EventArgs e)
        {
        ...
        private void ucSearchInOut_eventTagInput()
        {
            try
            {
                if (string.IsNullOrEmpty(ucPropertyInOut.rtbArchiveID.Text.Trim()))
                {
                    return;
                }
                ...
                if (ItemsPublic.IsDigitNumber(ucSearchInOut.rtbGpId.Text, -10))
                {
                    if ( ucSearchInOut.rtbTagId.Text.Trim().Count() > 3)
                    {
                        //decimal? arcId = decimal.Parse(ucSearchInOut.rtbGpId.Text.Trim());
                        decimal? arcId = decimal.Parse(ucPropertyInOut.rtbArchiveID.Text.Trim());
                        decimal? tagId =  decimal.Parse(ucSearchInOut.rtbTagId.Text.Trim());
                        resultTag =  objManagerTag.UpdateTag(arcId,tagId,false);
                    ...
                    {
                        ItemsPublic.ShowMessage("انجام عملیات تخصیص تگ ناموفق");
                    }
                }
                else
                {
                    ItemsPublic.ShowMessage("انجام عملیات ناموفق");
                }
